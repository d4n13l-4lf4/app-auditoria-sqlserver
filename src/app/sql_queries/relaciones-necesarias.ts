...
( select s1.* from 
  (	select CAST(tab.object_id AS varchar(255)) +'-'+ CAST(col.column_id AS varchar(255)) as 'COLUMN_IDE',
  ...
  (
  select CAST(tab.object_id AS varchar(255)) +'-'+ CAST(col.column_id AS varchar(255)) as 'FK_COL'
   from sys.tables as tab
  ...
  union 
  select CAST(ind.object_id AS varchar(255)) +'-'+ CAST(indc.column_id AS varchar(255)) as 'PK_COL' 
...
(
select CAST(ind.object_id AS varchar(255)) +'-'+ CAST(indc.column_id AS varchar(255)) as 'COLUMN_IDE',
 ...
 where
	COL_NAME(CAST(SUBSTRING(C1.COLUMN_IDE, 1, CHARINDEX('-', C1.COLUMN_IDE) - 1) as INT), CAST(SUBSTRING(C1.COLUMN_IDE, CHARINDEX('-', C1.COLUMN_IDE) + 1, 2) as INT))
 = COL_NAME( CAST(SUBSTRING(C2.COLUMN_IDE, 1, CHARINDEX('-', C2.COLUMN_IDE) - 1) as INT), CAST(SUBSTRING(C2.COLUMN_IDE, CHARINDEX('-', C2.COLUMN_IDE) + 1, 2) as INT))
